...
async function compressOne(file, maxWidth, quality) {
  ...
  // 先把文件整个读进内存，后续全部基于 buffer 操作，
  // 避免 sharp 直接持有文件路径的读句柄、同时又要往同一路径写入导致的占用冲突（Windows 上尤其容易触发）
  ...
  // 文件名是 .jpg/.png，但内容实际已经是 avif/webp/heif 这种高效格式：
  // 如果已经很小了，转成 jpeg 反而会变大，直接跳过，只在报告里提示格式不符
  const realFormat = meta.format; // 'jpeg' | 'png' | 'webp' | 'avif' | 'heif' | ...
  ...
  if (ext === '.png') {
    ...
    // .jpg/.jpeg：统一重编码成标准 JPEG（包括内容实际是 avif/webp/heif 但体积较大的情况，这时候转成压缩过的 jpeg 反而更小）
    pipeline = pipeline.jpeg({ quality, mozjpeg: true });
  }

  const outputBuffer = await pipeline.toBuffer();

  // 写入时先写到临时文件再原子替换，进一步避免 Windows 下的写入占用问题
